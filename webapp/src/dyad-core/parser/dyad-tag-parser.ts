 ...
 * Adjustments for the web app: electron-log removed, Supabase SQL and
 * Turbo-Edits search/replace parsers dropped (not part of the cloud MVP).
 * All regexes and escape handling for the core protocol
 * (dyad-write / rename / delete / add-dependency / chat-summary / command)
 * are unchanged.
 */
import { normalizePath } from "../shared/normalize-path";
import { unescapeXmlAttr, unescapeXmlContent } from "../shared/xml-escape";

const logger = { warn: (..._args: unknown[]) => {} }; // electron-log removed for web
